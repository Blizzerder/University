...
FindPathBFS::FindPathBFS()
{
	...
	m_parent = new COORDINATE *[m_size.x];  //new 空间
...
void FindPathBFS::InitMap(bool **chess) //初始化
{
	for (int i = 0;i < m_size.x; i++)
	{
	...
	while (!m_paths_queue.empty())
		m_paths_queue.pop();//清空队列
}

void FindPathBFS::FindPath(COORDINATE begin_point, COORDINATE end_point)
{
	queue<COORDINATE> tempque;
	m_visit[begin_point.x][begin_point.y] = true; //第一个点已访问
	tempque.push(begin_point); //输入第一个起始点
	...
	while (!tempque.empty()) 
	{
		...
		while (quesize--)
		{
			parent_point = tempque.front();//访问队首元素
			tempque.pop(); //移除队首元素
			if (parent_point.x == end_point.x && parent_point.y == end_point.y) //目标已抵达
			{
				return;
			}
			for (int i = 0; i < 4; i++)
			{
				...
				next_point.y = parent_point.y + direction[i][1]; //朝四个方向移动
				if (next_point.x < 0 ||
					(next_point.x > (80 - 1))||
					next_point.y < 0||
					(next_point.y > (40 - 1))||
